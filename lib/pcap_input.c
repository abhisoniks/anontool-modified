...
/*
   define a filter here.As of now we are only sniffing packet which have 
   private address. No packet with public address need to sniff.
   
*/
   struct bpf_program fp;
   bpf_u_int32 net=0;			
   char filter_exp[]="!ip host 10.129.26.53";

   if(pcap_compile(pkt_handler,&fp,filter_exp,0,net)==-1){
     fprintf(stderr,"could not parse filter expression");
 ...
 /*
    Run compiled filter
*/
	if(pcap_setfilter(pkt_handler,&fp)==-1){
...
/*
Following is call back function used in pcap_loop function to 
sniff packet. pcap_pkthdr is struct contating inf related to when packet was sniffed,
length of paccket etc. THis struct is defined in pcap.h file.
*/
